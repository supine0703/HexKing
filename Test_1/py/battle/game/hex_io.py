...
from .term import gray, blue, red, darkblue, darkred
...
def print_board(board):
    ...
    sys.stdout.write(' ' * 6 + hdr + '\n')
    for i in range(board.shape[0]):
        txt = '  ' * (i + 1)
        txt += format_rank(board[i], i)
        sys.stdout.write(txt)
        sys.stdout.write('\n')
    sys.stdout.write(' ' * 28 + hdr + '\n')
...
def format_rank(rank, i):
    hdr = termcolor(f'{12-(i + 1):<2d}', fg=Color(darkblue))
    txt = hdr + '  '
...
def format_move(board, move):
    ...
    return '{}{}'.format(chr(j + ord('a')), 11 - i)
