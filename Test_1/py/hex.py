...
class StoneString:
    def __init__(self, player: Player, stones, empty_nbs):
        self.player = player
        self.stones = set(stones)
        self.empty_nbs = set(empty_nbs)

    @classmethod
    def merge(cls, str1, str2):
        assert str1.player == str2.player
        new_stones = str1.stones | str2.stones
        empty_stones = (str1.empty_nbs | str1.empty_nbs) - new_stones
        return StoneString(str1.player, new_stones, empty_stones)
...
class HexBoard:
    def __init__(self, size: int = 11):
        assert 0 < size <= 19
        self.ORDER = size
        self.hash = EMPTY_BOARD
        self.stones = {
            NORTH: StoneString(Player.black, [NORTH], []),
            SOUTH: StoneString(Player.black, [SOUTH], []),
            EAST: StoneString(Player.white, [EAST], []),
            WEST: StoneString(Player.white, [WEST], [])
        }  # type: {(int, int): StoneString}
        self.state = {
            Player.black: np.zeros([self.ORDER, self.ORDER]),
            Player.white: np.zeros([self.ORDER, self.ORDER])
        }  # type: {Player: np.ndarray}

    def __str__(self):
        chess = [['', f"HASH: {self.hash}"], ['', ' '] + list(ascii_uppercase[:self.ORDER])]
        for row in range(self.ORDER):
            line = ['  ' * row, f"{row:2d}"]
            for col in range(self.ORDER):
                string = self.stones.get((row, col))
                line.append(STONE_LABEL.get(None if string is None else string.player))
            chess.append(line)
        return '\n'.join('   '.join(line) for line in chess)

    def set_chess(self, attacker: Player, move: (int, int)):
        assert (0, 0) <= move < (self.ORDER, self.ORDER)
        empty_moves = []  # 棋子的气
        same_strings = []  # 己方的串

        for m in self.nbs(move):
            string = self.stones.get(m)
            if string is None:  # 空 cell
                empty_moves.append(m)
            elif string.player == attacker:  # 己方占领
                if string not in same_strings:
                    same_strings.append(string)
            else:
                ...
        new_string = StoneString(attacker, [move], empty_moves)
        # 合并串
        for s in same_strings:
            new_string = StoneString.merge(new_string, s)
        for i in new_string.stones:
            self.stones[i] = new_string
        # 更新state和hash
        self.state[attacker][move[0]][move[1]] = 1
        self.hash ^= HASH_CODE.get((move, attacker))

    def nbs(self, move: (int, int)) -> [(int, int)]:
        def in_range(point):
            return 0 <= point[0] < self.ORDER and 0 <= point[1] < self.ORDER

        def add(move1, move2):
            return move1[0] + move2[0], move1[1] + move2[1]

        nbs = list(filter(in_range, (add(move, dm) for dm in DIR)))

        if len(nbs) < 6:
            if move[0] == 0:
                nbs.append(NORTH)
            elif move[0] == self.ORDER - 1:
                nbs.append(SOUTH)
            if move[1] == 0:
                nbs.append(WEST)
            elif move[1] == self.ORDER - 1:
                nbs.append(EAST)
        return nbs


STATE_LATITUDE = 1  # total latitude = STATE_LATITUDE * 2 + 1


class HexGame:
    def __init__(self, order=11):
        self.board = HexBoard(order)
        self.winner = None
        self.give_up_state = None
        self.attacker = Player.black
        self.all_hash = set()
        self.mv_history = []  # type: list[(Player, (int ,int))]
        self.state_deque = {
            Player.black: deque(
                [np.zeros([self.board.ORDER, self.board.ORDER])],
                maxlen=STATE_LATITUDE + 1),
            Player.white: deque(
                [np.zeros([self.board.ORDER, self.board.ORDER])],
                maxlen=STATE_LATITUDE + 1)
        }  # type: {Player: deque}
    ...
    def __str__(self):
        if not self.is_end():
            return f"now is {self.attacker} gaming!" + self.board.__str__()
        if self.give_up_state is None:
            chess = [['', f"{self.winner} is win!"]]
            win_dir = NORTH if self.winner == Player.black else WEST
            for row in range(self.board.ORDER):
                line = ['  ' * row]
                for col in range(self.board.ORDER):
                    line.append('#'
                                if self.board.stones.get((row, col)) is self.board.stones.get(win_dir)
                                else '-'
                                )
                chess.append(line)
            return '\n'.join('   '.join(line) for line in chess)
        else:
            return f"{self.attacker} is given up, {self.winner} win!"

    def is_end(self):
        return False if self.winner is None else True

    def set_move(self, move: (int, int)) -> bool:
        if self.winner is not None:
            return False
        if move == (-1, -1):
            self.give_up_state = self.attacker
            self.winner = self.attacker.other()
            self.mv_history.append((self.attacker, move))
            return True
        elif self.board.stones.get(move) is not None:
            return False
        elif 0 <= move[0] < self.board.ORDER and 0 <= move[1] < self.board.ORDER:
            self.state_deque[self.attacker].append(copy.deepcopy(self.board.state[self.attacker]))
            self.mv_history.append((self.attacker, move))  # 历史记录
            self.board.set_chess(self.attacker, move)  # 下棋
            self.all_hash.add(self.board.hash)  # 记录hash
            self.attacker = self.attacker.other()
            # 检测是否有获胜方
            if self.board.stones.get(NORTH) is self.board.stones.get(SOUTH):
                self.winner = Player.black
            elif self.board.stones.get(EAST) is self.board.stones.get(WEST):
    ...
    def empty_moves(self):
        return list(filter(
            lambda move: self.board.stones.get(move) is None,
            [(row, col) for row in range(self.board.ORDER) for col in range(self.board.ORDER)]
        ))


class HexAgent:
    def __init__(self, player: Player, a_game: HexGame):
        self.player = player  # type: Player
        self.game = a_game  # type: HexGame

    def run_a_move(self, how):
        if self.game.attacker == self.player:
            if how == 'R':
                moves = self.game.empty_moves()
                r = np.random.randint(-1, 9999)
                self.game.set_move((-1, -1) if r == -1 else moves[r % len(moves)])
            elif how == 'M':
                ...
            else:
                ...
