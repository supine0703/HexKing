                             ...
                             const InferiorCells& inf)
{
    ...
    for (int i = 0; i < BITSETSIZE; i++)
    {
        ...
        {
            Q_ASSERT(intersectSmallest.test(i));
    ...
    if (curMostHits == 0)
        qWarning() << "curMostHits == 0!!\n";
    Q_ASSERT(curMostHits > 0);
...
{
    Q_ASSERT(EndgameUtil::IsWonGame(brd, color));
...
{
    Q_ASSERT(EndgameUtil::IsLostGame(brd, color));

    // Determine if color's opponent has guaranteed win
    HexColor other = !color;

    //qDebug() << "Opponent has won; playing most blocking move.\n";
