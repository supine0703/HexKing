...
bitset_t ComputeDeadRegions(const Groups& groups)
{
    ...
    for (GroupIterator i(groups, NOT_EMPTY); i; ++i)
    {
        ...
        HexColor c = i->Color();
        Q_ASSERT(HexColorUtil::isBlackWhite(c));
    ...
    Q_ASSERT(BitsetUtil::IsSubsetOf(dead, brd.GetEmpty()));
    return dead;
}
        ...
        for (BitsetIterator y(empty); *y != *x; ++y) {
            ...
            for (BitsetIterator z(empty); z; ++z) {
                ...
                if ((xyExclusiveNbs & brd.GetBlack()).any()) {
                    ...
                    Q_ASSERT(BitsetUtil::IsSubsetOf(dead, empty));
                }
    ...
    for (BitsetIterator p(consider); p; ++p)
    {
        ...
        for (BoardIterator nb(brd.Const().Nbs(*p)); nb; ++nb)
        {
            ...
            else if (ncolor == color)
            {
                ...
                {
                    Q_ASSERT(!adj_to_edge || edgeNbr == cap);
                    adj_to_edge = true;
                    edgeNbr = cap;
                    cnbs.insert(cap);
        ...
        else if (adj_to_edge || cnbs.size() >= 2)
        {
            ...
            if (cnbs.size() == 1)
            {
                Q_ASSERT(adj_to_edge && enbs.size() == 1);
                inf.AddVulnerable(*p, *enbs.begin());
            }
            else
            {
                Q_ASSERT(!adj_to_edge ||
        ...
        else if (cnbs.size() == 1)
        {
            ...
            Q_ASSERT(enbs.size() == 1);
            Q_ASSERT(empty_adj_to_group.count() >= 2);
                                        ...
                                        bool only_around_last_move) const
{
    ...
    if (find_reversible)
    {
        Q_ASSERT(brd.IsColor(last_move, !color));
    ...
    //qDebug() << "  " << time.count() << "ms to find inferior cells.\n";
...
bool ICEngine::IsMonocolorUsingCapture(FillinMode mode)
{
    ...
    case MONOCOLOR_USING_CAPTURED : break;
