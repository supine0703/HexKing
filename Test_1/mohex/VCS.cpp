...
{
    Q_ASSERT(m_fulls_and_queue.IsEmpty());
    Q_ASSERT(m_semis_or_queue.IsEmpty());
}
...
void VCS::LoadCapturedSetPatterns()
{
    ...
    try {
        ...
        qDebug() << "VCS: reading captured set patterns from '" << file << "'.\n";
    }
    catch (SgException& e) {
        throw SgException("VCS: " + std::string(e.what()));
    }
    std::vector<Pattern> patterns;
    Pattern::LoadPatternsFromStream(inFile, patterns);
    //qDebug() << "VCS:: parsed " << patterns.size() << " patterns.\n";
...
{
    Q_ASSERT(false /* TODO implement copy constructor */);
}
...
void VCS::AddBaseVCs()
{
    ...
    for (GroupIterator x(*m_groups, not_other); x; ++x)
    {
        ...
        {
            Q_ASSERT(*y == m_groups->CaptainOf(*y));
            m_statistics.base_attempts++;
            Q_ASSERT(x->Captain() != *y);
                ...
                const Groups& groups, const PatternState& patterns)
{
    ...
    //qDebug() << "  " << time.count() << "ms to build vcs.\n";
}
...
{
    Q_ASSERT(x != y);
    Q_ASSERT((x_merged & y_merged).none());
...
{
    Q_ASSERT(x == m_groups->CaptainOf(x));
    Q_ASSERT(y == m_groups->CaptainOf(y));
    Q_ASSERT(x != y);
    Q_ASSERT(m_brd->GetColor(x) != !m_color);
    Q_ASSERT(m_brd->GetColor(y) != !m_color);
...
std::vector<bitset_t> VCOr(CarrierList semis, bitset_t cands,
...
HexPoint VCS::SemiKey(bitset_t carrier, HexPoint x, HexPoint y) const
{
    ...
    Q_ASSERT(false /* always should find a key */);
