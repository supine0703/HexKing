...
{
    //qDebug() << "VCPattern::CreatePatterns(" << width << ", " << height << ")\n";
    std::ifstream fin;
    try {
        std::string file = MiscUtil::OpenFile("vc-patterns.txt", fin);
        //qDebug() << "VCPattern: loading pattern templates from '" << file << "'.\n";
    }
    catch (SgException& e) {
        throw SgException("VCPattern: " + std::string(e.what()));
    }
    std::vector<VCPattern> out[BLACK_AND_WHITE];
    ...
    while (true)
    {
        ...
        Q_ASSERT(!carrier.empty());
        Q_ASSERT(patternHeight == -1
        ...
        {
            if (endpoint == SOUTH)
                throw SgException("Start pattern with no endpoint!");
    ...
    //qDebug() << "Combining start(" << start.size() << ") and end(" << end.size() << ")...\n";
    StoneBoard sb(width, height);
    ...
    //qDebug() << "Constructed " << numConstructed << ".\n" << "Parsed " << numComplete << " complete.\n";
    //qDebug() << "Translating, rotating, mirroring...\n";
    for (std::size_t i=0; i<complete.size(); ++i)
        ProcessPattern(complete[i], sb, out);
    //qDebug() << out[BLACK].size() << " total patterns\n";
