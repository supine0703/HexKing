                               ...
                               HexPoint point, HexDirection dir)
{
    ...
    int x, y;
    Q_ASSERT(HexPointUtil::isInteriorCell(point));
...
{
    Q_ASSERT((brd.Width() & 1) && (brd.Height() & 1));
...
HexPoint BoardUtil::RandomEmptyCell(const StoneBoard& brd)
{
    ...
    int randMove = QRandomGenerator::system()->bounded(count) + 1;
...
{
    Q_ASSERT(carrier.test(p1));
    Q_ASSERT(carrier.test(p2));
