...
const PatternMatcherData* PatternMatcherData::Get(const ConstBoard* brd)
{
    ...
    for (std::size_t i = 0; i < data.size(); ++i)
        if (*brd == *data[i]->brd)
            return data[i];
    //qDebug() << "Data does not exist. Creating...\n";
...
{
    //qDebug() << "PatternMatcherData::Initialize " << "(" << brd->Width() << " x " << brd->Height() <<")\n";
...
{
    Q_ASSERT(m_brd.Const().IsCell(cell));
...
void PatternState::Update(HexPoint cell)
{
    if (HexPointUtil::isSwap(cell))
        return;
    Q_ASSERT(m_brd.Const().IsLocation(cell));
...
{
    Q_ASSERT(m_brd.Const() == other.m_brd.Const());
